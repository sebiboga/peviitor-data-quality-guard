...
(async () => {
    ...
    // === SPECIAL: OLX/Publi24 → Use simple GET, if 200 keep + set vdate=today ===
    if (JOB_URL.includes('olx.ro') || JOB_URL.includes('publi24.ro')) {
        console.log('DETECTED: OLX/Publi24 link - simple GET check');
        ...
        try {
            ...
            const url = page.url();
            console.log('Final URL after redirect:', url);
            console.log('RESULT: ACTIVE (OLX/Publi24, page loaded)');
            console.log('DECISION: Set vdate=today, skip further validation');
            fs.writeFileSync('/tmp/url_status.txt', 'OLX_KEEP');
        } catch (e) {
            console.log('RESULT: EXPIRED (OLX/Publi24, cannot load)');
